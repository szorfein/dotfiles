...
ext_wall() {
  color=$(cat < "$filename" | jq ".wallpaper" | tr -d '"')
  echo "$color" | tr -d '"'
}

ext_dark() {
  color=$(cat < "$filename" | jq ".colors.m3$1" | tr -d '"')
  echo "$color" | tr -d '"'
}

ext_terminal() {
  color=$(cat < "$filename" | jq ".colors.$1" | tr -d '"')
  echo "$color" | tr -d '"'
}
...
surface_tint=$(ext_dark 'primary') # same Primary in dark
...
t_black=$(ext_terminal 'term0')
t_black_bright=$(ext_terminal 'term8')
t_red=$(ext_terminal 'term1')
t_red_bright=$(ext_terminal 'term9')
t_green=$(ext_terminal 'term2')
t_green_bright=$(ext_terminal 'term10')
t_yellow=$(ext_terminal 'term3')
t_yellow_bright=$(ext_terminal 'term11')
t_blue=$(ext_terminal 'term4')
t_blue_bright=$(ext_terminal 'term12')
t_magenta=$(ext_terminal 'term5')
t_magenta_bright=$(ext_terminal 'term13')
t_cyan=$(ext_terminal 'term6')
t_cyan_bright=$(ext_terminal 'term14')
t_white=$(ext_terminal 'term7')
t_white_bright=$(ext_terminal 'term15')
...
bright0=${t_black_bright:1} # bright black
