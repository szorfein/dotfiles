...
function music:progressbar()
    return wibox.widget({
        ...
        background_color = md.sys.color.secondary .. md.sys.elevation.level1,
    ...
    local top = wibox.widget({
        {
            {
                text = 'MPD',
                font = md.sys.typescale.title_medium.font .. ' ' .. md.sys.typescale.title_medium.size,
                widget = wibox.widget.textbox,
            },
            nil,
...
function music:artist()
    return wibox.widget({
        align = 'center',
        font = md.sys.typescale.body_small.font .. ' ' .. md.sys.typescale.body_small.size,
        forced_height = dpi(24),
        widget = wibox.widget.textbox,
    })
end
    ...
    return wibox.widget({
        button_icon({
            ...
            cmd = function()
                ...
                self.textbox_save:buttons(gears.table.join(awful.button({}, 1, function()
...
function music:playlist_prompt()
    awful.prompt.run({
      ...
      ']], function(_, stderr, _, exit_code)
                if exit_code == 0 then
                    naughty.notify({ title = 'Playlist', text = 'Saved to :  ' .. input })
                else
    ...
    return wibox.widget({
        {
            nil,
            {
                button_text({
                    ...
                    text = 'playlists',
                    cmd = function()
                        self.dialog_open_playlist:display()
                    end,
                }).widget,
                spacing = dpi(8),
                layout = wibox.layout.fixed.horizontal,
