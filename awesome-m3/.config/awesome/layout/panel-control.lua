...
function control:signals()
    ...
    awesome.connect_signal('daemon::geoloc', function(country, city)
        local _country = country and tostring(country) or 'nil'
        local _city = city and tostring(city) or 'Somewhere'
        self.geoloc.text = _city .. ', ' .. _country
    end)
...
function control:left_side()
    ...
    return wibox.widget({
                ...
                {
                    {
                        text = day,
                        align = 'left',
                        font = md.sys.typescale.display_large.font .. ' ' .. md.sys.typescale.display_large.size,
        ...
        { -- bottom
            self:centered({
                require('lib.card-elevated'):elevated(require('widgets.sysfetch')()),
            }),
            self:centered({
...
function control:progressbar(value)
    return wibox.widget({
        ...
        background_color = md.sys.color.primary .. md.sys.elevation.level1,
...
function control:right_side()
    ...
    local mfont = md.sys.typescale.headline_large.font .. ' ' .. md.sys.typescale.headline_large.size

    local on = md.sys.color.on_surface
    local on_variant = md.sys.color.on_surface_variant .. md.sys.state.disable_content_opacity
        ...
        {
            {
                ...
                },
                {
                    format = '%R.%p',
                    align = 'center',
                    font = md.sys.typescale.body_large.font .. ' ' .. dpi(14),
