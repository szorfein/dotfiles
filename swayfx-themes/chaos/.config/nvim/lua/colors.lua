-- theme chaos
-- base colors: https://github.com/catppuccin/nvim/blob/main/lua/catppuccin/palettes/mocha.lua
-- WGAG AAA need a contrast of 7:1 for normal text
return {
	pink = "#f986af", -- magenta light
	mauve = "#f46889", -- magenta
	red = "#fb5c67", -- red
	maroon = "#fe7378", -- red light
	peach = "#f37e4e", -- yellow
	yellow = "#ffad4f", -- yellow light
	green = "#edad74", -- green
	teal = "#ebca7c", -- green light
	sky = "#dd89fd", -- cyan light
	sapphire = "#c471e2", -- cyan
	blue = "#d358cb", -- blue
	lavender = "#f671b4", -- blue light
	text = "#F1DEDD",
	subtext1 = "#fcddf0",
	subtext0 = "#fccfe0", -- white
	-- ...
	--surface2 = '#322827'
	--surface1 = '#271D1D',
	surface0 = "#3D3231",
	base = "#1A1111",
	mantle = "#231919",
	--crust = '#140C0C',
	--crust = '#0E0E13', -- deep dark
}
